...
const DashboardUserList = () => {
  ...
  const openEditForm = (item) => {
    ...
    setEditState(item.admin);
  };

  // Update user fuction
  const handleUserUpdate = async (item) => {
  ...
  const handleUserDelete = async (item) => {
    const resoult = confirm(
      `Are you sure you wan to delete user '${item.name}'?`,
      PopupOptions
    );
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200  my-6 overflow-x-scroll xl:overflow-hidden">
        <table className="w-full border-collapse bg-white text-left text-sm text-gray-500">
          <thead className="bg-gray-50">
            <tr>
              <th scope="col" className="px-6 py-4 font-medium text-gray-400">
                Name
              </th>
              <th scope="col" className="px-6 py-4 font-medium text-gray-400">
                State
              ...
              allUsers?.map((item) => {
                return (
                  <React.Fragment key={item._id}>
                    <tr className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-3">
                        ...
                        >
                          <span
                            className={`${
                              item.admin ? "bg-green-600" : "bg-orange-500"
                    ...
                    <tr
                      className={`${
                        handleEdit && clickedItem === item._id
                          ? "visible border-red-300 w-full border-2 "
                          : "hidden"
                      } bg-red-50  `}
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="h-1.5 w-1.5 rounded-full bg-green-600">
                          <select
                            className="select select-bordered w-[80px] max-w-xs select-sm border-2 border-red-200 rounded-xl"
                            value={editState}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <button
                            onClick={() => handleUserUpdate(item)}
                            className={`${
                              isEditing
                                ? "bg-gray-300"
                                : "bg-pizza_wood-400 border-0 hover:bg-pizza_wood-300"
                            } btn btn-primary btn-sm text-white max-w-xs relative rounded-md`}
                            disabled={isEditing}
                          >
                            Edit
                            {isEditing && <Spinner />}
